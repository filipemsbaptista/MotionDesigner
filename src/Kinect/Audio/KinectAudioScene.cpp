...
int g_nXRes_ = 0, g_nYRes_ = 0;

const int jointScale = 10;
const int Zfactor = 2;


void KinectAudioScene::setup(string filePath){
	printf("Setting up Interactive Audio Scene!\n");
	ofEnableSmoothing();
...
//--------------------------------------------------------------
void KinectAudioScene::keyPressed(int key){
	switch (key){
	case ' ':
...
void KinectAudioScene::windowResized(int w, int h){
	marker = ofPoint(ofGetWidth()/2, ofGetHeight()/2, 1);
}

// ---------------------------------------------
void KinectAudioScene::drawSkeleton(){
	ofSetColor(ofColor::green);
		ofRect(headPos.x, headPos.y, headPos.z, jointScale, jointScale);
		ofRect(neckPos.x, neckPos.y, neckPos.z, jointScale, jointScale);
		ofRect(handLPos.x, handLPos.y, handLPos.z, jointScale, jointScale);
		ofRect(elbowLPos.x, elbowLPos.y, elbowLPos.z, jointScale, jointScale);
		ofRect(handRPos.x, handRPos.y, handRPos.z, jointScale, jointScale);
		ofRect(elbowRPos.x, elbowRPos.y, elbowRPos.z, jointScale, jointScale);
		ofRect(shoulderLPos.x, shoulderLPos.y, shoulderLPos.z, jointScale, jointScale);
		ofRect(shoulderRPos.x, shoulderRPos.y, shoulderRPos.z, jointScale, jointScale);
		ofRect(torsoPos.x, torsoPos.y, torsoPos.z, jointScale, jointScale);
		ofRect(hipLPos.x, hipLPos.y, hipLPos.z, jointScale, jointScale);
		ofRect(hipRPos.x, hipRPos.y, hipRPos.z, jointScale, jointScale);
		ofRect(kneeLPos.x, kneeLPos.y, kneeLPos.z, jointScale, jointScale);
		ofRect(kneeRPos.x, kneeRPos.y, kneeRPos.z, jointScale, jointScale);
		ofRect(footLPos.x, footLPos.y, footLPos.z, jointScale, jointScale);
		ofRect(footRPos.x, footRPos.y, footRPos.z, jointScale, jointScale);
...
void KinectAudioScene::setupGUI(){
	...
	ofAddListener(gui->newGUIEvent, this, &KinectAudioScene::guiEvent);
}

void KinectAudioScene::guiEvent(ofxUIEventArgs &e)
{
...
void KinectAudioScene::exit(){
	delete gui;
}
	
// ---------------------------------------------
void KinectAudioScene::setupKinect(){
	printf("Setting up KINECT for Interactive Audio Scene!\n");
...
void KinectAudioScene::updateKinectData(){
	...
	for (int i = 0; i < users.getSize(); i++)
	{
		...
		/*0*/	headPos = ofPoint(head.getPosition().x + ofGetWidth()/2, -(head.getPosition().y - ofGetHeight()/2), -head.getPosition().z/Zfactor);
		/*1*/	neckPos = ofPoint(neck.getPosition().x + ofGetWidth()/2, -(neck.getPosition().y - ofGetHeight()/2), -neck.getPosition().z/Zfactor);
		/*2*/	handLPos = ofPoint(handL.getPosition().x + ofGetWidth()/2, -(handL.getPosition().y - ofGetHeight()/2), -handL.getPosition().z/Zfactor);
		/*3*/	elbowLPos = ofPoint(elbowL.getPosition().x + ofGetWidth()/2, -(elbowL.getPosition().y - ofGetHeight()/2), -elbowL.getPosition().z/Zfactor);
		/*4*/	handRPos = ofPoint(handR.getPosition().x + ofGetWidth()/2, -(handR.getPosition().y - ofGetHeight()/2), -handR.getPosition().z/Zfactor);
		/*5*/	elbowRPos = ofPoint(elbowR.getPosition().x + ofGetWidth()/2, -(elbowR.getPosition().y - ofGetHeight()/2), -elbowR.getPosition().z/Zfactor);
		/*6*/	shoulderLPos = ofPoint(shoulderL.getPosition().x + ofGetWidth()/2, -(shoulderL.getPosition().y - ofGetHeight()/2), -shoulderL.getPosition().z/Zfactor);
		/*7*/	shoulderRPos = ofPoint(shoulderR.getPosition().x + ofGetWidth()/2, -(shoulderR.getPosition().y - ofGetHeight()/2), -shoulderR.getPosition().z/Zfactor);
		/*8*/	torsoPos = ofPoint(torso.getPosition().x + ofGetWidth()/2, -(torso.getPosition().y - ofGetHeight()/2), -torso.getPosition().z/Zfactor);
		/*9*/	hipLPos = ofPoint(hipL.getPosition().x + ofGetWidth()/2, -(hipL.getPosition().y - ofGetHeight()/2), -hipL.getPosition().z/Zfactor);
		/*10*/	hipRPos = ofPoint(hipR.getPosition().x + ofGetWidth()/2, -(hipR.getPosition().y - ofGetHeight()/2), -hipR.getPosition().z/Zfactor);
		/*11*/	kneeLPos = ofPoint(kneeL.getPosition().x + ofGetWidth()/2, -(kneeL.getPosition().y - ofGetHeight()/2), -kneeL.getPosition().z/Zfactor);
		/*12*/	kneeRPos = ofPoint(kneeR.getPosition().x + ofGetWidth()/2, -(kneeR.getPosition().y - ofGetHeight()/2), -kneeR.getPosition().z/Zfactor);
		/*13*/	footLPos = ofPoint(footL.getPosition().x + ofGetWidth()/2, -(footL.getPosition().y - ofGetHeight()/2), -footL.getPosition().z/Zfactor);
		/*14*/	footRPos = ofPoint(footR.getPosition().x + ofGetWidth()/2, -(footR.getPosition().y - ofGetHeight()/2), -footR.getPosition().z/Zfactor);
...
void KinectAudioScene::updateUserState(const nite::UserData& user, unsigned long long ts)
{
	if (user.isNew())
		USER_MESSAGE("New")
